            ...
            for item in instruments_data:
                try:
                    instrument = KiteInstrument(
                        ...
                        expiry=datetime.strptime(item['expiry'], '%Y-%m-%d').date() if item.get('expiry') and isinstance(item['expiry'], str) else None,
    ...
    async def get_quote(self, symbol: str) -> Optional[KiteQuote]:
        ...
        try:
            ...
            quote = KiteQuote(
                ...
                last_trade_time=datetime.strptime(data['last_trade_time'], '%Y-%m-%d %H:%M:%S') if data.get('last_trade_time') and isinstance(data['last_trade_time'], str) else datetime.now(),
